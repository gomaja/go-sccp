...
// ParameterType is a type for Parameter described in ITU-T Q.713 (03/01), section 4.
...
// ParameterNameCode is a type of Parameter Name Code defined in ITU-T Q.713 (03/01), Table 2.
...
func ParseOptionalParameters(b []byte) ([]Parameter, int, error) {
	...
	if offset == len(b) {
		if len(params) == 0 || params[len(params)-1].Code() != PCodeEndOfOptionalParameters {
			return nil, offset, io.ErrUnexpectedEOF
		}
	}
...
func (p *PartyAddress) valueLen() int {
	l := 1
	if p.HasPC() {
		l += 2
	}
...
// ITU-T Q.713 (03/01), section 3.20 permits 1 to 3952 octets of SCCP-user data.
const (
	minLongDataValueLen = 1
	maxLongDataValueLen = 3952
)
...
func (l *LongData) Read(b []byte) (int, error) {
	...
	if err := validateLongDataLength(l.length); err != nil {
		return 2, err
	}
	n := l.length + 2
	if len(b) < n {
...
func validateLongDataLength(length int) error {
	if length < minLongDataValueLen || length > maxLongDataValueLen {
		return fmt.Errorf("%s: invalid length: expected %d-%d, got %d", PCodeLongData, minLongDataValueLen, maxLongDataValueLen, length)
	}
	return nil
}
